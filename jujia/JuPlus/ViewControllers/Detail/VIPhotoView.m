...
- (instancetype)initWithFrame:(CGRect)frame andImage:(NSString *)image
{
    ...
    if (self) {
        ...
        [imageView setimageUrl:image placeholderImage:nil];
...
{
    //给图片添加点击事件
    UITapGestureRecognizer *singleGes=[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(tapHandler1:)];
    singleGes.numberOfTapsRequired = 1;
    [_containerView addGestureRecognizer:singleGes];

    UITapGestureRecognizer *doubleGes = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tapHandler2:)];
    doubleGes.numberOfTapsRequired = 2;
    [_containerView addGestureRecognizer:doubleGes];
    //doubleGes侦测失败之后才会触发singleGes（即为doubleGes的优先级最高），所以单击事件会有一个侦测延迟
...
{
    [[self getSuperViewController] dismissViewControllerAnimated:YES completion:^{
