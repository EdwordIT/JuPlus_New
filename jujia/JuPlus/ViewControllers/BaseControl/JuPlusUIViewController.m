...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    //token失效处理
    if (tag==ERROR_TOKEN_INVALID) {
        [[JuPlusUserInfoCenter sharedInstance] resetUserInfo];
        [self gologin];
           }
    //版本过低，强制更新
    else if(tag==ERROR_VERSON_OUT)
    {
        NSString* path=appUrl;
        [[UIApplication sharedApplication] openURL:[NSURL URLWithString:path]];
    }
    //单品或者套餐详情不存在
    else if (tag==ERROR_NO_COLLOCATION||tag==ERROR_NO_PRODUCT)
    {
        [self.navigationController popViewControllerAnimated:YES];
    }
}
...
-(void)viewWillDisappear:(BOOL)animated
{
    ...
    [MobClick endLogPageView:self.titleLabel.text];
}
-(void)viewWillAppear:(BOOL)animated
{
    
    [super viewWillAppear:animated];
    [MobClick beginLogPageView:self.titleLabel.text];
